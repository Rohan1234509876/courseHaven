...
const Purchases = () => {
  ...
  const token = user?.token?.token;  // extract JWT string


    const fetchPurchases = async () => {
      
    if(!token){
      setError("Please login to buy a course");
      return;
    }
    ...
    const response =await axios.get(`http://localhost:4001/api/v1/user/purchases`,
      ...
      setError(error.response.data.errors || "Error in fetching purchases");
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 bg-gray-100 p-5 transform  md:translate-x-0 transition-transform duration-300 ease-in-out w-64 z-50`}
      >
        <nav>
      ...
      {/* Main Content */}
      <div
        className={`flex-1 p-8 bg-gray-50 transition-all duration-300  md:ml-64`}
      >
